    ...
    gaia = mo.sql(
        ...
        WITH
            prep AS (
                SELECT
                    ...
                    *
                FROM
                    'https://pub-1da360b43ceb401c809f68ca37c7f8a4.r2.dev/data/gaia-5m.parquet'
                WHERE
                    parallax BETWEEN -5 AND 20
        ...
        SELECT
            (1.340264 * lambda * cos(t)) / (
                sqrt(3) / 2 * (
                    1.340264 + (-0.081106 * 3 * t2) + (t6 * (0.000893 * 7 + 0.003796 * 9 * t2))
                )
            ) AS u,
            t * (
                1.340264 + (-0.081106 * t2) + (t6 * (0.000893 + 0.003796 * t2))
            ) AS v,
            * EXCLUDE ('t', 't2', 't6')
        FROM
            prep
...
def _(gaia, spec):
    ...
    MosaicWidget(spec, data={"gaia": gaia.arrow()})
    return
